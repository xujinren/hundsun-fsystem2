<template>
	<!-- 文件夹的组件 -->
	<div id="xls" v-on:contextmenu="comRight_($event)" v-on:click="read_xls"  :title="'名称:' + item.f_name + '    '
		+ '创建日期:' + item.f_date
	"><!-- -->
		<div class="menu_rightClick" id="menu_right" v-if="menu_right_show">
			<p  v-on:click.stop="deleteN">删除</p>
		</div>
		<img  class="xls_img"  src="../../../../assets/xls.png"/>
		<br/>
...
<script>
	...
	export default{
		...
		methods:{
			...
			read_xls:function(){
				// let this_ = this;
				// this_.$emit("xlsSome", id);
				// 发送请求查询绝对路径
				let this_ = this;
				this_.$emit("xlsSome",this_.item.f_path);
